...
void splatSolidMesh(const dart::Mesh & mesh, dart::Grid3D<float> & sdf) {
    ...
    for (int f = 0; f<mesh.nFaces; f++) {
        ...
        for (int z = std::max(static_cast<float>(0.0),floor((min_z-sdf.offset.z)/sdf.resolution)); z < std::min(static_cast<float> (sdf.dim.z),ceil((max_z-sdf.offset.z)/sdf.resolution)); z++) {
            for (int y = std::max(static_cast<float>(0.0),floor((min_y-sdf.offset.y)/sdf.resolution)); y < std::min(static_cast<float> (sdf.dim.y),ceil((max_y-sdf.offset.y)/sdf.resolution)); y++) {
                for (int x = std::max(static_cast<float>(0.0),floor((min_x-sdf.offset.x)/sdf.resolution)); x < std::min(static_cast<float> (sdf.dim.x),ceil((max_x-sdf.offset.x)/sdf.resolution)); x++) {
